...
# the ezmeta user, and ensure file permissions, grant sudo rights, and such
...
if [ "$(id -u)" == 0 ] ; then
    ...
    # Refit the ezmeta user to the desired the user (NB_USER)
    if id ezmeta &> /dev/null ; then
        if ! usermod --home "/home/${NB_USER}" --login "${NB_USER}" ezmeta 2>&1 | grep "no changes" > /dev/null; then
            _log "Updated the ezmeta user:"
            _log "- username: ezmeta       -> ${NB_USER}"
            _log "- home dir: /home/ezmeta -> /home/${NB_USER}"
        fi
    elif ! id -u "${NB_USER}" &> /dev/null; then
        _log "ERROR: Neither the ezmeta user or '${NB_USER}' exists. This could be the result of stopping and starting, the container with a different NB_USER environment variable."
        exit 1
    fi
    ...
    # Move or symlink the ezmeta home directory to the desired users home
    # directory if it doesn't already exist, and update the current working
    # directory to the new location if needed.
    if [[ "${NB_USER}" != "ezmeta" ]]; then
        if [[ ! -e "/home/${NB_USER}" ]]; then
            _log "Attempting to copy /home/ezmeta to /home/${NB_USER}..."
            mkdir "/home/${NB_USER}"
            if cp -a /home/ezmeta/. "/home/${NB_USER}/"; then
                _log "Success!"
            else
                _log "Failed to copy data from /home/ezmeta to /home/${NB_USER}!"
                _log "Attempting to symlink /home/ezmeta to /home/${NB_USER}..."
                if ln -s /home/ezmeta "/home/${NB_USER}"; then
                    _log "Success creating symlink!"
                else
                    _log "ERROR: Failed copy data from /home/ezmeta to /home/${NB_USER} or to create symlink!"
                    exit 1
                fi
            fi
        fi
        # Ensure the current working directory is updated to the new path
        if [[ "${PWD}/" == "/home/ezmeta/"* ]]; then
...
# user we started as.
    ...
    ezmeta_UID="$(id -u ezmeta 2>/dev/null)"  # The default UID for the ezmeta user
    ezmeta_GID="$(id -g ezmeta 2>/dev/null)"  # The default GID for the ezmeta user
    ...
    if ! whoami &> /dev/null; then
        ...
        if [[ -w /etc/passwd ]]; then
            _log "Renaming old ezmeta user to nayvoj ($(id -u ezmeta):$(id -g ezmeta))"
            ...
            sed --expression="s/^ezmeta:/nayvoj:/" /etc/passwd > /tmp/passwd
            echo "${NB_USER}:x:$(id -u):$(id -g):,,,:/home/ezmeta:/bin/bash" >> /tmp/passwd
            ...
            if [[ "${NB_USER}" != "ezmeta" ]]; then
                _log "WARNING: user is ${NB_USER} but home is /home/ezmeta. You must run as root to rename the home directory!"
            fi
        else
    ...
    if [[ ! -w /home/ezmeta ]]; then
        _log "WARNING: no write access to /home/ezmeta. Try starting the container with group 'users' (100), e.g. using \"--group-add=users\"."
    fi
